...
export const joinWaitingList = mutation({
  ...
  handler: async (ctx, { eventId, userId }) => {
    // Check if the user already has an active entry in the waiting list for this event
...
export const purchaseTicket = mutation({
  ...
  handler: async (ctx, { ticket }) => {
    const { eventId, userId, waitingListId, purchasedAt, status } = ticket;

    // Log the start of the ticket purchase process
    ...
    try {
      ...
      if (waitingListEntry.userId !== userId) {
        console.error("User ID mismatch", { waitingListUserId: waitingListEntry.userId, requestUserId: userId });
      ...
      // Create the ticket without payment info
      console.log("Creating ticket for user and event");
      ...
      // Process queue for next person in the waiting list
      await processQueue(ctx, { eventId });

      console.log("Purchase ticket completed successfully");
    } catch (error) {
      console.error("Failed to complete ticket purchase:", error);
      throw new Error(`Failed to complete ticket purchase: ${error.message}`);
    }
  },
});
