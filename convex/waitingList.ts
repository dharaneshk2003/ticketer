  ...
  handler: async (ctx, { eventId }) => {
    ...
    for (const user of waitingUsers) {
      try {
        ...
        if (ctx.internal && typeof ctx.internal.runAfter === "function") {
          await ctx.internal.runAfter(
          ...
          console.error(
            `Failed to schedule expiration for user ${user._id}. 'runAfter' is not defined.`
          );
        }
      } catch (error) {
        console.error(
          `Error processing user ${user._id} in queue for event ${eventId}:`,
          error
        );
      }
    }
...
export const releaseTicket = mutation({
  ...
  handler: async (ctx, { eventId, waitingListId }) => {
    ...
    console.log("Releasing ticket:", { eventId, waitingListId, entry });

    // Delete the waiting list entry
    await ctx.db.delete(waitingListId);
    console.log("Deleted waiting list entry:", waitingListId);

    // Process queue to offer the ticket to the next person
    try {
      await processQueue(ctx, { eventId });
      console.log("Queue processed successfully for event:", eventId);
    } catch (error) {
      console.error("Error processing queue:", error);
      throw new Error("Failed to process queue for the next person");
    }
  },
});
